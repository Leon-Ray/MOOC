...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat(A.D, {(j, k): getitem(A, (j, k)) + getitem(B, (j, k)) for j in A.D[0] for k in A.D[1]})

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D, {(j, k): getitem(M, (j, k))*alpha for j in M.D[0] for k in M.D[1]})

def equal(A, B):
    "Returns true iff A is equal to B"
    assert A.D == B.D
    if False in [getitem(A, (j, k))==getitem(B, (j, k)) for j in A.D[0].union(B.D[0]) for k in A.D[1].union(B.D[1])]:
        return False
    else:
        return True
...
def transpose(M):
    ...
    return Mat((M.D[1], M.D[0]), {(k, j): getitem(M, (j, k)) for j in M.D[0] for k in M.D[1]})
...
def matrix_matrix_mul(A, B):
    ...
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(i, j): sum([A.f.get((i, k),0)*B.f.get((k, j),0) for k in A.D[1]])  for i in A.D[0] for j in B.D[1]})
